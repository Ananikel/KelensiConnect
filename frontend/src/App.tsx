...
// The following imports are commented/removed because the console logs indicated they were unused (TS6133).
...
import { Page, Member, Contribution, UserProfile, ChatMessage, AppEvent, Notification, Role, Permission, Photo, ContributionType, DocArticle } from './types';
...
  // Removed unused state variables: events, photos, contributionTypes, docArticles
...
    setError(null);
    try {
        // We only fetch data used by Dashboard, Members, Finances, Communication
        const data = await api.getAllData();
        setMembers(data.members);
        setContributions(data.contributions);
        setMessages(data.messages);
        // setEvents(data.events); // Removed unused
        // setPhotos(data.photos); // Removed unused
        setRoles(data.roles);
        setPermissions(data.permissions);
        // setContributionTypes(data.contributionTypes); // Removed unused
        // setDocArticles(data.docArticles); // Removed unused
    } catch (e) {
...
  // Added explicit type 'Member' for parameter 'memberData'
...
  // Added explicit type 'number' for parameter 'memberId' (Fixes TS7006 implicitly)
...
  // Removed unused Page names from the object literal: Cotisations, Événements, Galerie, Live, Documentation, Paramètres
...
      {/* Added explicit type 'number' for parameter 'id' (Fixes TS7006) */}
      <NotificationCenter notifications={notifications} removeNotification={(id: number) => setNotifications(n => n.filter(notif => notif.id !== id))} />
...
      {/* Added explicit type 'UserProfile' for parameter 'p' (Fixes TS7006) */}
      {isProfileModalOpen && <ProfileModal user={userProfile} onSave={(p: UserProfile) => { setUserProfile(p); setProfileModalOpen(false); }} onClose={() => setProfileModalOpen(false)} />}
    </div>
  );
};
