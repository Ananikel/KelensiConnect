            ...
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col">
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-4 overflow-y-auto">
                    ...
                    <div>
                        ...
                        <textarea id="doc-content" value={content} onChange={e => setContent(e.target.value)} rows={12} className="mt-1 block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-200 font-mono text-sm" placeholder="Vous pouvez utiliser des sauts de ligne pour la mise en forme." required />
                    </div>
                </form>
                <div className="mt-auto bg-gray-50 dark:bg-gray-700/50 px-6 py-4 flex justify-end space-x-3 border-t dark:border-gray-700">
                    ...
                    <button type="submit" formAction="submit" onClick={handleSubmit} className="px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700">Enregistrer</button>
                </div>
            </div>
        </div>
    );
...
const Documentation: React.FC<DocumentationProps> = ({ articles, onSaveArticle, onDeleteArticle }) => {
    ...
    const selectedArticle = useMemo(() => {
        ...
        // If the selected article is filtered out, clear selection
    ...
    const handleSave = (articleData: DocArticle) => {
        ...
        setModalOpen(false);
        // If it's a new article, select it
        if (!articleToEdit) {
    ...
    const handleDeleteConfirm = () => {
        if (articleToDelete) {
            ...
            // If the deleted article was the selected one, select the first available article
