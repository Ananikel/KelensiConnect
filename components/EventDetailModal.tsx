            ...
            <div className="bg-white rounded-lg shadow-xl w-full max-w-3xl max-h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
                ...
                <div className="p-6 overflow-y-auto">
                    ...
                    <div className="space-y-6">
                        <div>
                            <h4 className="text-lg font-semibold text-green-700 mb-2">Participants ({rsvpsByStatus['Attending'].length})</h4>
                            <div className="space-y-1">
                                {rsvpsByStatus['Attending'].map(member => <MemberRow key={member.id} member={member} currentStatus="Attending" />)}
                            </div>
                        </div>
                         <div>
                            <h4 className="text-lg font-semibold text-yellow-700 mb-2">Peut-être ({rsvpsByStatus['Maybe'].length})</h4>
                            <div className="space-y-1">
                               {rsvpsByStatus['Maybe'].map(member => <MemberRow key={member.id} member={member} currentStatus="Maybe" />)}
                            </div>
                        </div>
                         <div>
                            <h4 className="text-lg font-semibold text-red-700 mb-2">Ne participent pas ({rsvpsByStatus['Not Attending'].length})</h4>
                            <div className="space-y-1">
                                {rsvpsByStatus['Not Attending'].map(member => <MemberRow key={member.id} member={member} currentStatus="Not Attending" />)}
                            </div>
                        </div>
                         <div>
                            <h4 className="text-lg font-semibold text-gray-600 mb-2">Pas de réponse ({rsvpsByStatus['No Response'].length})</h4>
                            <div className="space-y-1">
